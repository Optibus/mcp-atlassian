...
export async function createConfluencePageV2(
  config: AtlassianConfig,
  params: { spaceId: string; title: string; content: string; parentId?: string }
): Promise<any> {
  // Validate content
  if (!params.content.trim().startsWith("<")) {
    throw new Error(
      "Content must be in Confluence storage format (XML-like HTML)."
    );
  }
  // Prepare payload
...
// Can update title or body or both. Must increment version on each update.
export async function updateConfluencePageV2(
  config: AtlassianConfig,
  params: { pageId: string; title: string; content: string; version: number }
): Promise<any> {
  const payload = {
    id: params.pageId,
    status: "current",
...
export async function addConfluenceCommentV2(
  config: AtlassianConfig,
  params: { pageId: string; content: string }
): Promise<any> {
  if (!params.content.trim().startsWith("<")) {
    throw new Error(
      "Comment content must be in Confluence storage format (XML-like HTML)."
    );
  }
  const requestData = {
...
export async function deleteConfluencePageV2(
  config: AtlassianConfig,
  params: { pageId: string; draft?: boolean; purge?: boolean }
): Promise<any> {
  const query: string[] = [];
  ...
  return await callConfluenceApi<any>(config, endpoint, "DELETE");
}

// Update Confluence page title (API v2)
export async function updateConfluencePageTitleV2(
  config: AtlassianConfig,
  params: { pageId: string; title: string; version: number }
): Promise<any> {
  const payload = {
    title: params.title,
...
export async function updateConfluenceFooterCommentV2(
  config: AtlassianConfig,
  params: {
...
export async function deleteConfluenceFooterCommentV2(
  config: AtlassianConfig,
  commentId: string | number
): Promise<any> {
  return await callConfluenceApi<any>(
