...
export const createPageSchema = z.object({
  ...
  content: z.string()
    .describe(`Content of the page (required, must be in Confluence storage format - XML-like HTML).
...
export async function createPageHandler(
  ...
  try {
    logger.info(
      `Creating new page (v2) "${params.title}" in spaceId ${params.spaceId}`
    );
    ...
    logger.error(
      `Error creating page (v2) in spaceId ${params.spaceId}:`,
      error
    );
...
export const registerCreatePageTool = (server: McpServer) => {
  server.tool(
    "createPage",
    "Create a new page in Confluence (API v2, only supports spaceId)",
