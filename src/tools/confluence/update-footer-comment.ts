...
export const updateFooterCommentSchema = z.object({
  commentId: z
    ...
    .describe("ID of the comment to update (required)"),
  version: z
    .number()
    .describe(
  ...
  representation: z
    .string()
    .optional()
    .describe('Content representation, default is "storage"'),
...
export async function updateFooterCommentHandler(
  ...
  try {
    ...
    return {
      ...
      message: `Footer comment ${params.commentId} updated to version ${data.version?.number} successfully.`,
    };
  } catch (error) {
    if (error instanceof ApiError) throw error;
    logger.error(
      `Error updating footer comment (v2) with ID ${params.commentId}:`,
      error
    );
    throw new ApiError(
      ApiErrorType.SERVER_ERROR,
      `Failed to update footer comment: ${
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                success: false,
                message: `Error while updating footer comment: ${
                  error instanceof Error ? error.message : String(error)
                }`,
              }),
            },
