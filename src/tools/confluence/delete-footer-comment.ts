...
export const deleteFooterCommentSchema = z.object({
  commentId: z
    ...
    .describe("ID of the comment to delete (required)"),
});
...
export async function deleteFooterCommentHandler(
  ...
  try {
    ...
    return {
      success: true,
      id: params.commentId,
      message: `Footer comment ${params.commentId} has been permanently deleted.`,
    };
  } catch (error) {
    if (error instanceof ApiError) throw error;
    logger.error(
      `Error deleting footer comment (v2) with ID ${params.commentId}:`,
      error
    );
    throw new ApiError(
      ApiErrorType.SERVER_ERROR,
      `Failed to delete footer comment: ${
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                success: false,
                message: `Error while deleting footer comment: ${
                  error instanceof Error ? error.message : String(error)
                }`,
              }),
            },
