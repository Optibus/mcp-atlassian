...
export const updatePageSchema = z.object({
  ...
  content: z.string().optional()
    .describe(`New content of the page (Confluence storage format only, XML-like HTML).
...
export async function updatePageHandler(
  ...
  try {
    ...
    // Get current version, title, content if missing
    const baseUrl = config.baseUrl.endsWith("/wiki")
      ? config.baseUrl
      : `${config.baseUrl}/wiki`;
    ...
    if (!content) {
      // Get current body if content not provided
      const bodyRes = await fetch(`${url}/body`, {
        method: "GET",
        headers,
        credentials: "omit",
      });
      if (!bodyRes.ok)
        throw new Error(`Failed to get page body: ${params.pageId}`);
      const bodyData = await bodyRes.json();
      content = bodyData.value;
      if (!content) throw new Error("Missing content for page update");
    }
    // Call helper updateConfluencePageV2 with all fields
    ...
    throw new ApiError(ApiErrorType.SERVER_ERROR, message, 500);
  }
}
