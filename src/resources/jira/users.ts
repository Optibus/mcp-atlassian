...
export function registerUserResources(server: McpServer) {
        ...
        contents: [
          {
            ...
            text: JSON.stringify({
              message:
                "Please use a more specific user resource. The Jira API requires parameters to search users.",
              suggestedResources: [
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "Get details for a specific Jira user by accountId. Replace {accountId} with the user accountId.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      ...
      try {
        ...
        normalizedAccountId = Array.isArray(params.accountId)
          ? params.accountId[0]
          : params.accountId;
        ...
        const uriString = typeof uri === "string" ? uri : uri.href;
        // Normalize metadata/schema for user detail resource
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "List assignable users for a Jira project. Replace {projectKey} with the project key.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        const config = Config.getAtlassianConfigFromEnv();
        const projectKey = Array.isArray(params.projectKey)
          ? params.projectKey[0]
          : params.projectKey;
        ...
        const url = `${baseUrl}/rest/api/3/user/assignable/search?project=${encodeURIComponent(
          projectKey
        )}`;
        logger.info(
          `Getting assignable users for project ${projectKey}: ${url}`
        );
        const response = await fetch(url, {
          method: "GET",
          headers,
        ...
        resources: [
          {
            ...
            description:
              "List users by role in a Jira project. Replace {projectKey} and {roleId} with the project key and role ID.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        if (!projectKey || !roleId)
          throw new Error("Missing projectKey or roleId");
        const auth = Buffer.from(`${config.email}:${config.apiToken}`).toString(
          "base64"
        );
        const headers = {
        ...
        logger.info(
          `Getting users in role for project ${projectKey}, role ${roleId}: ${url}`
        );
        const response = await fetch(url, {
          method: "GET",
          headers,
