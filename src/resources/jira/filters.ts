...
export function registerFilterResources(server: McpServer) {
  ...
  // Only register each template once with handler
  ...
  const filterDetailsTemplate = new ResourceTemplate(
    "jira://filters/{filterId}",
    {
      list: async (_extra) => ({
        resources: [
          {
            ...
            description:
              "Get details for a specific Jira filter by ID. Replace {filterId} with the filter ID.",
            mimeType: "application/json",
          },
        ],
      }),
  ...
  // Register template with execution handler - only register once per URI
  server.resource(
    "jira-filters-list",
    filtersTemplate,
  ...
  server.resource(
    "jira-filter-details",
    filterDetailsTemplate,
    async (uri: string | URL, params: Record<string, any>, _extra: any) => {
      try {
        // Get config from environment
        const config = Config.getAtlassianConfigFromEnv();

        const filterId = Array.isArray(params.filterId)
          ? params.filterId[0]
          : params.filterId;
        ...
        logger.error(
          `Error getting filter details for filter ${params.filterId}:`,
          error
        );
        throw error;
      }
