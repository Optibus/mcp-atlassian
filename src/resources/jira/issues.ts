...
export function registerIssueResources(server: McpServer) {
  ...
  server.resource(
    ...
    async (uri, params, _extra) => {
      try {
        ...
        logger.info(
          `Searching Jira issues with JQL: ${jqlQuery || "All issues"}`
        );
        const response = await searchIssuesApi(config, jqlQuery, limit);

        // Format issues data
        const formattedIssues = response.issues.map((issue: any) =>
  ...
  server.resource(
    ...
    async (uri, params, _extra) => {
      try {
        const config = Config.getAtlassianConfigFromEnv();
        let normalizedIssueKey = Array.isArray(params.issueKey)
          ? params.issueKey[0]
          : params.issueKey;
  ...
  server.resource(
          ...
          resources: [
            {
              ...
              description:
                "List available transitions for a Jira issue. Replace {issueKey} with the issue key.",
              mimeType: "application/json",
            },
          ],
        };
    ...
    async (uri, params, _extra) => {
      try {
        ...
        logger.info(
          `Getting transitions for Jira issue: ${normalizedIssueKey}`
        );
  ...
  server.resource(
          ...
          resources: [
            {
              ...
              description:
                "List comments for a Jira issue. Replace {issueKey} with the issue key.",
              mimeType: "application/json",
            },
          ],
        };
    ...
    async (uri, params, _extra) => {
      try {
        ...
        const commentData = await getIssueComments(
          config,
          normalizedIssueKey,
          offset,
          limit
        ...
        const formattedComments = (commentData.comments || []).map((c: any) =>
          formatCommentData(c)
        );
