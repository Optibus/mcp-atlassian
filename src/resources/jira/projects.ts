        ...
        resources: [
          {
            ...
            description:
              "Get details for a specific Jira project by key. Replace {projectKey} with the project key.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        if (params && "projectKey" in params) {
          normalizedProjectKey = Array.isArray(params.projectKey)
            ? params.projectKey[0]
            : params.projectKey;
        }
        ...
        logger.info(
          `Getting details for Jira project: ${normalizedProjectKey}`
        );
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "List roles for a Jira project. Replace {projectKey} with the project key.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        const uriString = typeof uri === "string" ? uri : uri.href;
        // Normalize metadata/schema (using array of role objects, inline schema)
