        ...
        resources: [
          {
            ...
            description:
              "Get details for a specific Jira board by ID. Replace {boardId} with the board ID.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        return Resources.createStandardResource(
          ...
          1,
          0,
          `${config.baseUrl}/jira/software/projects/${
            board.location?.projectKey || "browse"
          }/boards/${boardId}`
        );
      } catch (error) {
        logger.error(
          `Error getting board details for board ${params.boardId}:`,
          error
        );
        throw error;
      }
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "List issues in a Jira board. Replace {boardId} with the board ID.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        logger.error(
          `Error getting issues for board ${params.boardId}:`,
          error
        );
        throw error;
      }
        ...
        resources: [
          {
            ...
            description:
              "Get configuration of a specific Jira board. Replace {boardId} with the board ID.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      try {
        ...
        const boardId = Array.isArray(params.boardId)
          ? params.boardId[0]
          : params.boardId;
        // Call API to get board configuration
        const response = await fetch(
          `${config.baseUrl}/rest/agile/1.0/board/${boardId}/configuration`,
          {
            method: "GET",
            headers: {
        ...
        const uriString = typeof uri === "string" ? uri : uri.href;
        // Inline schema (basic description, no deep validation)
        ...
        logger.error(
          `Error getting board configuration for board ${params.boardId}:`,
          error
        );
        throw error;
      }
