...
import {
  getConfluenceSpacesV2,
  getConfluenceSpaceV2,
...
export function registerSpaceResources(server: McpServer) {
    ...
    async (uri, params, _extra) => {
      ...
      const metadata = {
        ...
        links: {
          ...
          next:
            hasMore && nextCursor
              ? `${uriString}?cursor=${encodeURIComponent(
                  nextCursor
                )}&limit=${limit}`
              : undefined,
        },
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "Get details for a specific Confluence space by id. Replace {spaceId} with numeric space id (e.g. 19464200).",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      const config = Config.getAtlassianConfigFromEnv();
      let normalizedSpaceId = Array.isArray(params.spaceId)
        ? params.spaceId[0]
        : params.spaceId;
      if (!normalizedSpaceId) throw new Error("Missing spaceId in URI");
      if (!/^\d+$/.test(normalizedSpaceId))
        throw new Error("spaceId must be a number");
      logger.info(
        `Getting details for Confluence space (v2) by id: ${normalizedSpaceId}`
      );
      // Get space info via API helper (assumes getConfluenceSpaceV2 supports lookup by id)
  ...
  server.resource(
        ...
        resources: [
          {
            ...
            description:
              "List all pages in a specific Confluence space. Replace {spaceId} with numeric space id.",
            mimeType: "application/json",
          },
        ],
      }),
    ...
    async (uri, params, _extra) => {
      ...
      if (!/^\d+$/.test(normalizedSpaceId))
        throw new Error("spaceId must be a number");
      // No longer lookup by key, use id directly
      const filterParams = {
